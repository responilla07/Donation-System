    ...
    // try {
      ...
      if (textController.text.length > 0) {
        if(lastDocument != null){
          if (isPullUp.value) {
            querySnapshot = await FirebaseFirestore.instance.collection("Products").where("searchKey", arrayContains: query).where("ownerID", isEqualTo: loggedUser.uid).orderBy('dateCreated', descending: true).limit(6).startAfterDocument(lastDocument).get();
          ...
          querySnapshot = await FirebaseFirestore.instance.collection("Products").where("searchKey", arrayContains: query).where("ownerID", isEqualTo: loggedUser.uid).orderBy('dateCreated', descending: true).limit(6).get();
          List<ItemModel> listChar = List<ItemModel>();
          for (var i = 0; i < querySnapshot.docs.length; i++) {
            listChar.add(ItemModel(querySnapshot.docs[i].id, querySnapshot.docs[i].data()));
          }
        ...
        if(lastDocument != null){
          if (isPullUp.value) {
            querySnapshot = await FirebaseFirestore.instance.collection("Products").where("ownerID", isEqualTo: loggedUser.uid).orderBy('dateCreated', descending: true).limit(6).startAfterDocument(lastDocument).get();
            if (querySnapshot.docs.length > 0) {
              List<ItemModel> listChar = List<ItemModel>();
              for (var i = 0; i < querySnapshot.docs.length; i++) {
                listChar.add(ItemModel(querySnapshot.docs[i].id, querySnapshot.docs[i].data()));
          ...
          isFetching.value = true;
          listOfItems.value.clear();
          querySnapshot = await FirebaseFirestore.instance.collection("Products").where("ownerID", isEqualTo: loggedUser.uid).orderBy('dateCreated', descending: true).limit(6).get();
          List<ItemModel> listChar = List<ItemModel>();
          for (var i = 0; i < querySnapshot.docs.length; i++) {
            listChar.add(ItemModel(querySnapshot.docs[i].id, querySnapshot.docs[i].data()));
          }
